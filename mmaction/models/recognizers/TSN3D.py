...
class TSN3D(BaseRecognizer):
    ...
    def extract_feat(self, img_group, temp_att):
        x = self.backbone(img_group, temp_att)
        return x
    ...
    def forward_train(self,
        ...
        x = self.extract_feat(img_group, None)
        ax, rx, att, t_att = x
        if self.necks is not None:
    ...
    def forward_test(self,
        ...
        print("backbone ...")
        if self.flip:
            img_group = self.extract_feat(torch.flip(img_group, [-1]), temp_att)
        x = self.extract_feat(img_group, temp_att)
        ax, rx, att, t_att = x
